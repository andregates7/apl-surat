...
$result = $koneksi->query("SELECT surat_masuk.no_agenda, surat_masuk.no_surat, surat_masuk.asal_surat, disposisi.tujuan, surat_keluar.no_suratkeluar,surat_keluar.tgl_surat FROM surat_masuk LEFT JOIN disposisi ON surat_masuk.id = disposisi.id_surat LEFT JOIN surat_keluar ON disposisi.id = surat_keluar.id_dispo WHERE surat_keluar.tgl_surat BETWEEN '$awal' AND '$akhir' ORDER BY no_agenda ASC;")->fetch_all(MYSQLI_ASSOC);
...
$pdf->Cell(80);
$pdf->Cell(30,10,'Rekap Agenda Surat '.$nama_bulan,0,0,'C');
$pdf->Ln(12);

//table Head
$pdf->SetFont('Arial','B',10);
$pdf->SetWidths(array(15,40,45,30,30,30));
$pdf->SetAligns(array('C','C','C','C','C','C'));
$pdf->Row(array('No. Agenda','No. Surat','Asal Surat','Tujuan Disposisi','No. Surat Keluar','Tanggal Surat Keluar'));
...
foreach ($result as $row) 
{
    ...
    $tujuan               = $row["tujuan"];
    $no_keluar            = $row["no_suratkeluar"];

    //Now show the columns
    $pdf->SetFont('Arial','',10);
    $pdf->SetAligns(array('C','L','L','L','C','C'));
    $pdf->Row(array($no_agenda,$no_surat,$asal_surat,$tujuan,$no_keluar,$tgl_surat));
}
...
$pdf->Output('Laporan Rekap Agenda Surat','I');
?>